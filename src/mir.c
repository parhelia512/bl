...
#define INVALID_ENUM_TYPE_INFER_MESSAGE "Unknown enum expression type. Unable to infer enum type from the usage."

typedef sarr_t(LLVMTypeRef, 8) llvm_types_t;

static batomic_s64 _instr_id_counter = 1;
...
static void fn_dtor(struct mir_fn *fn) {
	bmagic_assert(fn);
	if (fn->dyncall_ext && fn->dyncall_ext->extern_callback_handle) dcbFreeCallback(fn->dyncall_ext->extern_callback_handle);
	arrfree(fn->variables);
	tbl_free(fn->phi_block_mapping);
	sarrfree(&fn->defer_stack);
}

static void fn_poly_dtor(struct mir_fn_recipe *recipe) {
...
static struct mir_codegen *duplicate_codegen(struct context *ctx, struct mir_codegen *orig);
static struct mir_codegen *swap_current_codegen(struct context *ctx, struct mir_codegen *codegen);
...
#define create_type_struct(ctx, ...) _create_type_struct((ctx), &(create_type_struct_args_t){__VA_ARGS__})
...
static struct mir_type *_create_type_enum(struct context *ctx, create_type_enum_args_t *args);
#define create_type_enum(ctx, ...) _create_type_enum((ctx), &(create_type_enum_args_t){__VA_ARGS__})
...
#define create_fn(ctx, ...) _create_fn((ctx), &(create_fn_args_t){__VA_ARGS__})
static struct mir_fn_group  *create_fn_group(struct context *ctx, struct ast *decl_node, mir_fns_t *variants);
static struct mir_fn_recipe *create_fn_recipe(struct context *ctx, struct ast *ast_lit_fn, enum mir_fn_recipe_flavor flavor);
...
static struct mir_arg *_create_arg(struct context *ctx, create_arg_args_t *args);
#define create_arg(ctx, ...) _create_arg((ctx), &(create_arg_args_t){__VA_ARGS__})
...
static struct mir_instr_block *create_block(struct context *ctx, struct ast *node, const str_t name);
// Create and append block into function specified.
static struct mir_instr_block *append_block(struct context *ctx, struct ast *node, struct mir_fn *fn, const str_t name, const bool is_unreachable);
...
static struct mir_instr_block *set_block_insertion_cursor(struct mir_instr_block *block);
...
static struct mir_instr *create_instr_const_int(struct context *ctx, struct ast *node, struct mir_type *type, u64 val, bool is_volatile_type);
...
typedef struct
{
	...
	u32                  index;
	enum builtin_id_kind builtin_id;
} create_instr_member_ptr_args_t, append_instr_member_ptr_args_t;
static struct mir_instr *_create_instr_member_ptr(struct context *ctx, create_instr_member_ptr_args_t *args);
#define create_instr_member_ptr(ctx, ...) _create_instr_member_ptr((ctx), &(create_instr_member_ptr_args_t){__VA_ARGS__})
...
} create_instr_call_args_t, append_instr_call_args_t;
...
static struct mir_instr *append_instr_unroll(struct context *ctx, struct ast *node, struct mir_instr *src, u32 index, bool force_tmp);
...
static struct mir_instr *_append_instr_member_ptr(struct context *ctx, append_instr_member_ptr_args_t *args);
#define append_instr_member_ptr(ctx, ...) _append_instr_member_ptr((ctx), &(append_instr_member_ptr_args_t){__VA_ARGS__})
...
static struct mir_instr *_append_instr_call(struct context *ctx, append_instr_call_args_t *args);
#define append_instr_call(ctx, ...) _append_instr_call((ctx), &(append_instr_call_args_t){__VA_ARGS__})
...
static struct mir_instr *_append_instr_decl_var(struct context *ctx, append_instr_decl_var_args_t *args);
#define append_instr_decl_var(ctx, ...) _append_instr_decl_var((ctx), &(append_instr_decl_var_args_t){__VA_ARGS__})
...
static struct mir_instr *_create_instr_decl_var_impl(struct context *ctx, create_instr_decl_var_impl_args_t *args);
#define create_instr_decl_var_impl(ctx, ...) _create_instr_decl_var_impl((ctx), &(create_instr_decl_var_impl_args_t){__VA_ARGS__})

static struct mir_instr *_append_instr_decl_var_impl(struct context *ctx, append_instr_decl_var_impl_args_t *args);
#define append_instr_decl_var_impl(ctx, ...) _append_instr_decl_var_impl((ctx), &(append_instr_decl_var_impl_args_t){__VA_ARGS__})
...
static struct mir_instr *append_instr_const_int(struct context *ctx, struct ast *node, struct mir_type *type, u64 val, bool is_volatile_type);
...
static struct mir_instr       *ast_stmt_defer(struct context *ctx, struct ast *defer);
...
typedef struct
{
	...
	enum ast_flags       flags;
	enum builtin_id_kind builtin_id;
} ast_expr_lit_fn_args_t;

static struct mir_instr *_ast_expr_lit_fn(struct context *ctx, ast_expr_lit_fn_args_t *args);
#define ast_expr_lit_fn(ctx, ...) _ast_expr_lit_fn((ctx), &(ast_expr_lit_fn_args_t){__VA_ARGS__})
...
static void          analyze_instr_block_finalize(struct context *ctx, struct mir_instr_block *block);
...
static struct result analyze_instr_cond_insert(struct context *ctx, struct mir_instr_cond_insert *cond_insert);
static struct result analyze_instr_defer(struct context *ctx, struct mir_instr_defer *defer);
static struct result analyze_instr_defer_insert(struct context *ctx, struct mir_instr_defer_insert *defer_insert);
...
static inline struct mir_var *add_global_int(struct context *ctx, struct id *id, struct ast *node, bool is_mutable, bool is_builtin, struct mir_type *type, s32 v) {
	return add_global_variable(ctx, id, node, is_mutable, is_builtin, create_instr_const_int(ctx, NULL, type, v, false));
}
...
static inline void usage_check_push(struct context *ctx, struct scope_entry *entry) {
	...
	switch (entry->kind) {
	case SCOPE_ENTRY_FN:
		if (isflag(entry->as.fn->flags, FLAG_TEST_FN)) return;
		if (isflag(entry->as.fn->flags, FLAG_MAYBE_UNUSED)) return;
		if (entry->as.fn->recipe_ext) return;
		break;
	case SCOPE_ENTRY_VAR:
		if (isflag(entry->as.var->flags, FLAG_MAYBE_UNUSED)) return;
		break;
	default:
		return;
	}
...
static inline bool can_type_mutate_comptime_to_const(struct mir_type *type) {
	bassert(type);
	switch (type->kind) {
	case MIR_TYPE_TYPE:
	case MIR_TYPE_INT:
	...
	case MIR_TYPE_PLACEHOLDER:
		return true;
	case MIR_TYPE_SLICE:
		return type->data.strct.is_string_literal;
...
static inline bool can_mutate_comptime_to_const(struct context *ctx, struct mir_instr *instr) {
	...
	return can_type_mutate_comptime_to_const(instr->value.type);
}
...
static inline void lock_type_cache(struct context *ctx) {
	mtx_lock(&ctx->mir->type_cache_lock);
}

static inline void unlock_type_cache(struct context *ctx) {
...
static inline struct mir_type *lookup_type(struct context *ctx, hash_t hash, str_t name) {
	...
	if (i != -1) {
		type = ctx->mir->type_cache[i].type;
	}
	mtx_unlock(&ctx->mir->type_cache_lock);
	return_zone(type);
}
...
static inline struct mir_instr_block *ast_current_block(struct context *ctx) {
	return ctx->codegen->current_block;
}

static inline struct mir_fn *ast_current_fn(struct context *ctx) {
	return ctx->codegen->current_block ? ctx->codegen->current_block->owner_fn : NULL;
}

static inline void terminate_block(struct mir_instr_block *block, struct mir_instr *terminator) {
	bassert(block);
	bassert(terminator);
	bassert(!block->terminal && "Block is already terminated!");
...
static inline bool is_last_instr_in_block(struct mir_instr *instr) {
	return instr->owner_block && instr->owner_block->last_instr == instr;
}

// Mutate instruction to a new kind, keeps the base, but zero init destination kind related part.
...
#endif
	instr->kind        = kind;
	const usize offset = sizeof(struct mir_instr);
	memset((u8 *)instr + offset, 0, SIZEOF_MIR_INSTR - offset);
	return instr;
}

static struct mir_instr *duplicate_instr(struct context *ctx, struct mir_instr *instr) {
	bassert(instr);
...
static inline void erase_instr(struct mir_instr *instr) {
	...
	if (instr->owner_block) {
		...
		if (block->last_instr == instr) block->last_instr = instr->prev;
		if (block->terminal == instr) block->terminal = NULL;
	}
...
static inline void add_phi_income(struct mir_instr_phi *phi, struct mir_instr *value, struct mir_instr_block *block) {
	bassert(value && block);
	const s32 index = phi->num;
	bassert(index < (s32)static_arrlenu(phi->incoming_values));
...
static inline void analyze_schedule(struct context *ctx, struct mir_instr *instr) {
	bassert(instr);
	mtx_lock(&ctx->analyze->stack_lock);
	++push_count;
	arrput(analyze_current(ctx), instr);
	mtx_unlock(&ctx->analyze->stack_lock);
}
...
static inline str_t _unique_name(struct context *ctx, char *prefix_ptr, s32 prefix_len) {
	...
	return_zone(scprint(ctx->string_cache, "{str}.{u64}", tmp, ui++));
}
...
static inline void set_current_block(struct context *ctx, struct mir_instr_block *block) {
	if (block && block->owner_fn) set_block_insertion_cursor(block);
	ctx->codegen->current_block = block;
}

struct mir_insertion_context {
	struct mir_instr_block *start_block;
	struct mir_instr       *terminator;
	struct mir_instr       *continue_instr;
	struct mir_codegen     *prev_codegen;
};
...
static inline void insertion_begin(struct context *ctx, struct mir_insertion_context *ictx, struct mir_instr *after_instr, struct mir_codegen *codegen) {
	...
	ictx->continue_instr = after_instr->next;
...
static inline void insertion_end(struct context *ctx, struct mir_insertion_context *ictx) {
	while (ictx->continue_instr) {
		// @Node 2026-01-26: Defer insert instruction might land in .unreachable block, this will produce false-positive
		//                   warnings even if there is no actual unreachable code following this instruction. Thus we
		//                   skip it here in case the current block is terminated -> unreachable block will be created on
		//                   `append_current_block` call.
		if (ictx->continue_instr->kind == MIR_INSTR_DEFER_INSERT && is_block_terminated(ictx->continue_instr->owner_block)) {
			ictx->continue_instr = ictx->continue_instr->next;
			erase_instr(ictx->continue_instr);
			continue;
		}
		append_current_block(ctx, ictx->continue_instr);
		ictx->continue_instr = ictx->continue_instr->next;
	}

	struct mir_instr_block *end_block = ast_current_block(ctx);
	if (!is_block_terminated(end_block) && ictx->terminator) {
		append_current_block(ctx, ictx->terminator);
		terminate_block(end_block, ictx->terminator);
	}

	// @Note 2026-01-27: This mapping is needed due to phi instruction, since we can insert any arbitrary code,
	//                   we might end up with different block structure after insertion. Thus any phi instruction
	//                   after insertion must adjust income blocks.
	if (ictx->start_block != end_block) {
		struct mir_fn *fn    = ictx->start_block->owner_fn;
		const s32      index = tbl_lookup_index(fn->phi_block_mapping, ictx->start_block);
		if (index == -1) {
			struct block_entry entry = {.hash = ictx->start_block, .value = end_block};
			tbl_insert(fn->phi_block_mapping, entry);
		} else {
			fn->phi_block_mapping[index].value = end_block;
		}
	}

	swap_current_codegen(ctx, ictx->prev_codegen);
}
...
static inline void commit_fn(struct context *ctx, struct mir_fn *fn) {
	...
	bmagic_assert(entry);
	bassert(entry->kind == SCOPE_ENTRY_INCOMPLETE);
...
static inline void provide_builtin_member(struct context *ctx, struct scope *scope, struct mir_member *member, bool commit) {
...
struct mir_codegen *duplicate_codegen(struct context *ctx, struct mir_codegen *orig) {
	...
	memcpy(codegen, orig, sizeof(*codegen));
	return codegen;
}

struct mir_codegen *swap_current_codegen(struct context *ctx, struct mir_codegen *codegen) {
	bassert(codegen);
	struct mir_codegen *tmp = ctx->codegen;
	ctx->codegen            = codegen;
	return tmp;
}
...
struct scope_entry *register_symbol(struct context *ctx, struct ast *node, struct id *id, struct scope *scope, bool is_builtin) {
	...
	scope_lock(scope);
	const hash_t scope_layer = ctx->codegen->current_scope_layer;
	...
	// no collision
	struct scope_entry *entry = scope_create_entry(ctx->scope_thread_local, .id = id, .node = node, .is_builtin = is_builtin);
	scope_insert(scope, scope_layer, entry);
...
struct mir_type *lookup_builtin_type(struct context *ctx, enum builtin_id_kind kind) {
	...
	struct mir_var *var = found->as.var;
...
struct mir_fn *lookup_builtin_fn(struct context *ctx, enum builtin_id_kind kind) {
	...
	ref_instr(found->as.fn->prototype);
	return found->as.fn;
}
...
struct mir_var *add_global_variable(struct context *ctx, struct id *id, struct ast *node, bool is_mutable, bool is_builtin, struct mir_instr *initializer) {
	...
	struct mir_instr_block *prev_block = ast_initializer_block_begin(ctx, NULL);
	{
		...
		append_instr_set_initializer(ctx, NULL, decl_var, initializer);
	}
...
struct mir_type *create_type_struct_incomplete(struct context *ctx, struct id *user_id, bool has_base) {
	...
	const hash_t     hash   = strhash(name);
	struct mir_type *result = lookup_type(ctx, hash, str_buf_view(name)); // Locked implicitly.
	if (result) {
		goto DONE;
	}
...
struct mir_type *_create_type_struct(struct context *ctx, create_type_struct_args_t *args) {
	struct id        id;
	struct mir_type *result;
...
struct mir_type *create_type_slice(struct context *ctx, enum mir_type_kind kind, struct id *user_id, struct mir_type *elem_ptr_type, bool is_string_literal) {
	...
	sarrput(members, tmp);
	provide_builtin_member(ctx, body_scope, tmp, true);
...
static struct mir_var *create_var_impl(struct context *ctx, create_var_impl_args_t *args) {
	...
	setflagif(tmp->iflags, MIR_VAR_GLOBAL, args->is_global || args->is_global_inline_init);
	setflagif(tmp->iflags, MIR_VAR_GLOBAL_INLINE_INIT, args->is_global_inline_init);
...
struct mir_fn *_create_fn(struct context *ctx, create_fn_args_t *args) {
...
struct mir_fn_recipe *create_fn_recipe(struct context *ctx, struct ast *ast_lit_fn, enum mir_fn_recipe_flavor flavor) {
	bassert(ast_lit_fn && ast_lit_fn->kind == AST_EXPR_LIT_FN);
	bassert(flavor != 0);
	struct mir_fn_recipe *tmp = arena_alloc(&ctx->mir_arenas->fn_recipe);
	bmagic_set(tmp);
...
struct mir_arg *_create_arg(struct context *ctx, create_arg_args_t *args) {
...
static inline void report_unreachable_block(struct context *ctx, struct mir_instr_block *block) {
	...
	const str_t debug_replacement = fn->instance_ext ? fn->instance_ext->debug_replacement_types : str_empty;
...
void append_current_block(struct context *ctx, struct mir_instr *instr) {
	...
	if (is_block_terminated(block)) {
		block = append_block(ctx, NULL, block->owner_fn, cstr(".unreachable"), true);
		set_current_block(ctx, block);
		append_instr_unreachable(ctx, instr->node);
		report_unreachable = true;
	}

	// @Note 2026-01-26: We allow reinsertion of erased instructions, this is kinda handy in cases
	//                   where we need to remove the instruction just temporarily and then append it back.
	bassert(instr->state == MIR_IS_PENDING || instr->state == MIR_IS_ERASED);
	instr->state = MIR_IS_PENDING;
...
struct mir_instr *_create_instr_member_ptr(struct context *ctx, create_instr_member_ptr_args_t *args) {

	struct mir_instr_member_ptr *tmp = create_instr(ctx, MIR_INSTR_MEMBER_PTR, args->node);
	tmp->target_ptr                  = ref_instr(args->target_ptr);
	tmp->scope_entry                 = args->scope_entry;

	if (args->ident) {
		tmp->id_kind  = MIR_INSTR_MEMBER_ID_IDENT;
		tmp->id.ident = args->ident;
		assert(args->index == 0);
		assert(args->builtin_id == BUILTIN_ID_NONE);
	} else if (args->builtin_id != BUILTIN_ID_NONE) {
		tmp->id_kind       = MIR_INSTR_MEMBER_ID_BUILTIN;
		tmp->id.builtin_id = args->builtin_id;
		assert(args->index == 0);
		assert(!args->ident);
	} else {
		tmp->id_kind  = MIR_INSTR_MEMBER_ID_INDEX;
		tmp->id.index = args->index;
		assert(args->builtin_id == BUILTIN_ID_NONE);
		assert(!args->ident);
	}
...
struct mir_instr *create_instr_const_int(struct context *ctx, struct ast *node, struct mir_type *type, u64 val, bool is_volatile_type) {
	bassert(!is_volatile_type || type->kind == MIR_TYPE_INT);
	struct mir_instr_const *tmp      = create_instr(ctx, MIR_INSTR_CONST, node);
	tmp->base.value.type             = type;
	tmp->base.value.addr_mode        = MIR_VAM_RVALUE;
	tmp->base.value.is_comptime      = true;
	tmp->base.value.is_type_volatile = is_volatile_type;
...
struct mir_instr_block *create_block(struct context *ctx, struct ast *node, const str_t name) {
	bassert(name.len);
	struct mir_instr_block *tmp = create_instr(ctx, MIR_INSTR_BLOCK, node);
...
struct mir_instr_block *append_block(struct context *ctx, struct ast *node, struct mir_fn *fn, const str_t name, const bool is_unreachable) {
	bassert(fn);

	struct mir_instr_block *block = create_block(ctx, node, name);
	block->owner_fn               = fn;
	block->is_unreachable         = is_unreachable;

	if (!fn->insertion_cursor_block) {
		bassert(!fn->entry_block && "Function entry block already set!");
		fn->entry_block = block;
		// first block is referenced every time!!!
		ref_instr(&fn->entry_block->base);
		goto DONE;
	}

	block->base.prev = &fn->insertion_cursor_block->base;
	block->base.next = fn->insertion_cursor_block->base.next;
	if (fn->insertion_cursor_block->base.next) {
		fn->insertion_cursor_block->base.next->prev = &block->base;
	}
	fn->insertion_cursor_block->base.next = &block->base;

DONE:
	fn->insertion_cursor_block = block;
	return block;
}

// Sets global block cursor so any new blocks are created after 'block'.
struct mir_instr_block *set_block_insertion_cursor(struct mir_instr_block *block) {
	bassert(block);
	bassert(block->owner_fn && "Block without owner function cannot be used as an insertion cursor.");
	struct mir_instr_block *prev_insertion_cursor_block = block->owner_fn->insertion_cursor_block;
	block->owner_fn->insertion_cursor_block             = block;
	return prev_insertion_cursor_block;
}
...
struct mir_instr *append_instr_set_initializer(struct context *ctx, struct ast *node, struct mir_instr *dest, struct mir_instr *src) {
	...
	// Used later to generate IR of the initializer block only in case the variable is used.
...
struct mir_instr *_append_instr_type_struct(struct context *ctx, append_instr_type_struct_args_t *args) {
	...
	tmp->is_multi_value               = args->is_multi_value;
...
struct mir_instr *append_instr_unroll(struct context *ctx, struct ast *node, struct mir_instr *src, u32 index, bool force_tmp) {
...
struct mir_instr *_create_instr_call(struct context *ctx, create_instr_call_args_t *args) {
	bassert(args->callee);
	struct mir_instr_call *tmp  = create_instr(ctx, MIR_INSTR_CALL, args->node);
	tmp->base.value.addr_mode   = MIR_VAM_RVALUE;
	tmp->base.value.is_comptime = args->is_comptime;
	tmp->args                   = args->args;
	tmp->callee                 = ref_instr(args->callee);
	tmp->is_inside_recipe       = args->is_inside_recipe;
	// reference all arguments
	for (usize i = 0; i < sarrlenu(args->args); ++i) {
		ref_instr(sarrpeek(args->args, i));
	}
...
struct mir_instr *create_default_value_for_type(struct context *ctx, struct mir_type *type) {
	...
	case MIR_TYPE_INT: {
		default_value = create_instr_const_int(ctx, NULL, type, 0, false);
		break;
	}
...
struct mir_instr *append_instr_cond_br(struct context *ctx, struct ast *node, struct mir_instr *cond, struct mir_instr_block *then_block, struct mir_instr_block *else_block, const bool is_static) {
...
struct mir_instr *_append_instr_member_ptr(struct context *ctx, append_instr_member_ptr_args_t *args) {
	struct mir_instr *tmp = _create_instr_member_ptr(ctx, args);
	append_current_block(ctx, tmp);
	return tmp;
}
...
struct mir_instr *append_instr_cond_insert(struct context *ctx, struct ast *node, struct mir_instr *cond, struct ast *then_block, struct ast *else_block) {
	bassert(then_block); // @Note 2026-01-17: Else block is optional.
	struct mir_instr_cond_insert *tmp = create_instr(ctx, MIR_INSTR_COND_INSERT, node);
	tmp->base.ref_count               = 0; // @Note 2026-01-18: Removed everytime after analyze.
	tmp->base.value.type              = ctx->builtin_types->t_void;
	tmp->base.value.addr_mode         = MIR_VAM_RVALUE;
	tmp->base.value.is_comptime       = true;
...
struct mir_instr *append_instr_defer(struct context *ctx, struct ast *node, struct ast *code) {
	bassert(code);
	struct mir_instr_defer *tmp = create_instr(ctx, MIR_INSTR_DEFER, node);
	tmp->base.ref_count         = 0; // @Note 2026-01-18: Removed everytime after analyze.
	tmp->base.value.type        = ctx->builtin_types->t_void;
	tmp->base.value.addr_mode   = MIR_VAM_RVALUE;
	tmp->base.value.is_comptime = true;
...
struct mir_instr *
_append_instr_decl_ref(struct context *ctx, append_instr_decl_ref_args_t *args) {
	struct mir_instr_decl_ref *tmp = create_instr(ctx, MIR_INSTR_DECL_REF, args->node);
	tmp->scope                     = args->scope;
	tmp->scope_layer               = args->scope_layer;
	tmp->rid                       = args->rid;
...
struct mir_instr *_append_instr_call(struct context *ctx, append_instr_call_args_t *args) {
	struct mir_instr *tmp = _create_instr_call(ctx, args);
	append_current_block(ctx, tmp);
	return tmp;
}
...
struct mir_instr *_create_instr_decl_var_impl(struct context *ctx, create_instr_decl_var_impl_args_t *args) {
	struct mir_instr_decl_var *tmp = create_instr(ctx, MIR_INSTR_DECL_VAR, args->node);
	tmp->base.value.type           = ctx->builtin_types->t_void;
	tmp->base.ref_count            = MIR_NO_REF_COUNTING;
	tmp->type                      = ref_instr(args->type);
	...
	tmp->var = create_var_impl(ctx,
	                           &(create_var_impl_args_t){
	                               ...
	                               .is_global_inline_init = args->is_global_inline_init,
...
struct mir_instr *_append_instr_decl_var_impl(struct context *ctx, append_instr_decl_var_impl_args_t *args) {
	struct mir_instr *tmp = _create_instr_decl_var_impl(ctx, args);
	if (args->is_global) {
...
inline struct mir_instr *append_instr_const_int(struct context *ctx, struct ast *node, struct mir_type *type, u64 val, bool is_volatile_type) {
	struct mir_instr *tmp = create_instr_const_int(ctx, node, type, val, is_volatile_type);
	append_current_block(ctx, tmp);
	return tmp;
}
...
struct mir_instr *append_instr_const_string(struct context *ctx, struct ast *node, str_t str) {
	...
	mir_instrs_t *values = arena_alloc(ctx->small_array_arena);

	struct mir_instr *len = create_instr_const_int(ctx, node, ctx->builtin_types->t_s64, str.len, false);
...
#define erase(i) \
	...
	while (sarrlenu(&queue)) {
		...
		case MIR_INSTR_COND_INSERT: {
			struct mir_instr_cond_insert *cond_insert = (struct mir_instr_cond_insert *)top;
			erase(cond_insert->cond);
			break;
		}
...
struct result analyze_instr_phi(struct context *ctx, struct mir_instr_phi *phi) {
	...
	struct mir_fn          *fn              = phi_owner_block->owner_fn;
	...
	for (s32 i = 0; i < phi->num; ++i) {
		...
		// @Node 2026-01-20: This complication was introduced by conditional compilatio (static if) where analyze
		//                   pass might completely change block hierarchy in case static if is present in the function.
		//                   We essentially have to override incoming block in case it changes.
		//
		//                   There might be better solution to this, however I tried to encapsulate inserted code into
		//                   dedicated branch, but since the original block might get split in the middle the issue
		//                   is still present.
		const s32 index = tbl_lookup_index(fn->phi_block_mapping, phi->incoming_blocks[i]);
		if (index != -1) {
			phi->incoming_blocks[i] = fn->phi_block_mapping[index].value;
		}
...
struct result analyze_instr_unroll(struct context *ctx, struct mir_instr_unroll *unroll) {
	...
	@Note 2026-02-24: In general, the unroll is supposed to pick element at index of multi-value return result of call. However there
	                  there are other usecases as well. One important thing here is that call expression might be replaced by constant
	                  value in case it's known at compile-time. Also there is special usage requred by 'catch' error semantics.

	                  Unroll will be generated when source type is composit multi-value type, when source is used more than once:
	                  a, b :: return_single_value(), or unroll forces temporary variable generation (this is a special case for 'catch'
	                  expression; the call result containing error might be used at any arbitrary place in the error handling block,
	                  thus we need to store it into temporary declaration).

	                  Temporary is required in general for unroll to work properly, the source function must be called only once and
	                  we have to distribute all returned values into individual destinations.

	                  *Cases*
	                  a, b :: 10;                   | Translates to: a :: 10; b :: a;.
	                  a    :: single_value();       | Call used once, unroll removed.
	                  a    :: single_value#();      | Call (converted to const) used once, unroll removed.
	                  a, b :: single_value();       | Call used 2 times, generates temporary.
	                  a, b :: single_value#();      | Call (converted to const) used 2 times, generates temporary.

	                  a    :: multi_value();        | Call is multi-value, generates temporary.
	                  a, b :: multi_value();        | Call is multi-value, generates temporary.
	                  a    :: multi_value#();       | Call (converted to const) is multi-value, generates temporary.
	                  a, b :: multi_value#();       | Call (converted to const) is multi-value, generates temporary.

	                  single_value() catch;         | Forced to generate temporary.
	                  e :: single_value() catch;    | Forced to generate temporary.
	                  multi_value() catch;          | Forced to generate temporary.
	                  a, e :: multi_value() catch;  | Forced to generate temporary.

	*/

	zone();
	struct mir_instr *src   = unroll->src;
	u32               index = unroll->index;

	bassert(src && "Missing unroll input!");
	bassert(index >= 0 || index == (u32)UNROLL_LAST_INDEX);
	bassert(src->value.type);

	// @Note 2026-02-24: Invalid global variable initialization is handled later in 'analyze_instr_set_initializer'.
	const bool is_global  = !scope_is_local(unroll->base.node->owner_scope);
	const bool is_invalid = is_global && !mir_is_comptime(src);

	struct mir_type *src_type         = src->value.type;
	const bool       is_multi_value   = mir_is_composite_type(src_type) && src_type->data.strct.is_multi_value;
	const bool       is_unroll_needed = unroll->force_tmp || is_multi_value || (src->kind == MIR_INSTR_CALL && src->ref_count > 1) || (src->kind == MIR_INSTR_CONST && src->ref_count > 1);
	if (!is_unroll_needed || is_invalid) {
		// @Note 2026-02-19: Unroll will be removed in 'analyze_stage_unroll' pass.

		unroll->index                  = 0;
		unroll->base.value.type        = src_type;
	...
	if (!tmp_var) {
		tmp_var = create_instr_decl_var_impl(ctx,
		                                     .node                  = unroll->base.node,
		                                     .name                  = unique_name(ctx, IMPL_TMP),
		                                     .init                  = src,
		                                     .is_global_inline_init = is_global);
		insert_instr_after(src, tmp_var);
		const struct result r = analyze_instr(ctx, tmp_var);
		if (r.state != ANALYZE_PASSED) {
			return_zone(r);
		}
		src->tmp_var = tmp_var;
	}
	...
	if (is_multi_value) {
		...
		if (index == (u32)UNROLL_LAST_INDEX) {
			// @Note 2026-02-05: In case of UNROLL_LAST_INDEX we're supposed to set index to the last element, this is used for error
			//                   capture.
			index = ((u32)sarrlenu(src_type->data.strct.members) - 1);
		}

		if (src->kind == MIR_INSTR_DECL_VAR) {
			// @Note 2026-02-24: Referenced right here, because it's explicitly used in following member_ptr.
			src = ref_instr(create_instr_decl_direct_ref(ctx, unroll->base.node, src));
			insert_instr_before(&unroll->base, src);
			analyze_instr_rq(ctx, src);
		}

		struct mir_instr_member_ptr *member_ptr = mutate_instr(&unroll->base, MIR_INSTR_MEMBER_PTR);
		member_ptr->id_kind                     = MIR_INSTR_MEMBER_ID_INDEX;
		member_ptr->id.index                    = index;
		member_ptr->target_ptr                  = src;

	} else {

		struct mir_instr_decl_direct_ref *ref = mutate_instr(&unroll->base, MIR_INSTR_DECL_DIRECT_REF);
		ref->ref                              = ref_instr(src);
	}

	// @Note 2026-02-19: Unroll was mutated at this point and needs to be analyzed again, thus we postpone here.
	bassert(unroll->base.kind != MIR_INSTR_UNROLL);
...
static struct result analyze_instr_compound_regular(struct context *ctx, struct mir_instr_compound *cmp) {
	...
	case MIR_TYPE_STRUCT: {
		...
		ints_t     *value_member_mapping = arena_alloc(ctx->small_array_arena);
		...
		for (usize i = 0; i < sarrlenu(values); ++i, ++last_member_index) {
			...
			if ((*value_ref)->kind == MIR_INSTR_DESIGNATOR) {
				...
				scope_lookup(ctx->assembly, scope, &(scope_lookup_args_t){
				                                       .id    = id,
				                                       .layer = type->data.strct.scope_layer,
				                                   },
				             &found,
				             1);
				if (!found) {
				...
				last_member_index = found->as.member->index;
...
static struct result analyze_instr_compound_implicit(struct context *ctx, struct mir_instr_compound *cmp) {
	...
	for (usize i = 0; all_comptime && i < sarrlenu(values); ++i) {
		all_comptime &= mir_is_comptime(sarrpeek(values, i));
	}
	cmp->base.value.is_comptime = all_comptime;
	return_zone(PASS);
}
...
struct result analyze_instr_set_initializer(struct context *ctx, struct mir_instr_set_initializer *si) {
	zone();
	bassert(si->dest && "Expected at least one variable.");
	// Just pre-scan to check if all destination variables are analyzed.
	bassert(si->dest->kind == MIR_INSTR_DECL_VAR);
	if (si->dest->state != MIR_IS_COMPLETE) {
		return_zone(POSTPONE); // PERFORMANCE: use wait???
	}

	struct mir_type *type = ((struct mir_instr_decl_var *)si->dest)->var->value.type;
	...
	if (!mir_is_comptime(si->src)) {
		report_error(EXPECTED_COMPTIME, si->dest->node, "Global variables must be initialized with compile-time known value.");
		if (si->src->kind == MIR_INSTR_CALL) {
			report_note(si->src->node, "Consider use of compile-time call '#()'.");
		}
	...
	struct mir_var *var = ((struct mir_instr_decl_var *)si->dest)->var;
	...
	var->value.is_comptime = !isflag(var->iflags, MIR_VAR_MUTABLE);
...
struct result analyze_instr_elem_ptr(struct context *ctx, struct mir_instr_elem_ptr *elem_ptr) {
	...
	if (require_tmp) {
		...
		struct mir_instr      *tmp_var = call->base.tmp_var;
		if (!tmp_var) {
			tmp_var = create_instr_decl_var_impl(ctx,
			                                     .name = unique_name(ctx, IMPL_TMP),
			                                     .init = arr_ptr);
...
#define IS_BUILTIN_MEMBER(kind) (member_ptr->id_kind == MIR_INSTR_MEMBER_ID_BUILTIN && member_ptr->id.builtin_id == (kind)) || \
	                                (member_ptr->id_kind == MIR_INSTR_MEMBER_ID_IDENT && is_builtin(member_ptr->id.ident, (kind)))
	...
	if (target_type->kind == MIR_TYPE_NAMED_SCOPE) {
		...
		struct scope *scope = scope_entry->as.scope;
		bmagic_assert(scope);

		bassert(member_ptr->id_kind == MIR_INSTR_MEMBER_ID_IDENT);
		struct id   *rid         = &member_ptr->id.ident->data.ident.id;
		struct unit *parent_unit = member_ptr->id.ident->location->unit;
		bassert(rid);
		bassert(parent_unit);
		...
		decl_ref->scope_layer               = ctx->codegen->current_scope_layer;
	...
	if (target_type->kind == MIR_TYPE_ARRAY) {
		bassert(member_ptr->id_kind != MIR_INSTR_MEMBER_ID_INDEX);

		// check array builtin members
		if (IS_BUILTIN_MEMBER(BUILTIN_ID_ARR_LEN)) {
		...
		} else if (IS_BUILTIN_MEMBER(BUILTIN_ID_ARR_PTR)) {
			// @Incomplete <2022-06-21 Tue> I don't remember why we need both checks here.
			// .ptr -> This will be replaced by:
			//     elemptr
			//     addrof
			// to match syntax: &array[0]
			struct mir_instr *index = create_instr_const_int(ctx, NULL, ctx->builtin_types->t_s64, 0, false);
			...
			bassert(member_ptr->id_kind == MIR_INSTR_MEMBER_ID_IDENT);
			report_error(INVALID_MEMBER_ACCESS, member_ptr->id.ident, "Unknown member.");
			return_zone(FAIL);
		}
	...
	if (target_type->kind == MIR_TYPE_TYPE) {
		...
		if (sub_type->kind == MIR_TYPE_ENUM) {
			...
			scope_lookup(ctx->assembly, scope, &(scope_lookup_args_t){
			                                       .layer = scope_layer,
			                                       .id    = &ident->data.ident.id,
			                                   },
			             &found,
			             1);
			if (!found) {
				report_error(UNKNOWN_SYMBOL, ident, "Unknown enumerator variant.");
				return_zone(FAIL);
			}
			...
			member_ptr->base.value.type        = found->as.variant->value_type;
			member_ptr->base.value.addr_mode   = target_addr_mode;
			...
			scope_lookup(ctx->assembly, scope, &(scope_lookup_args_t){
			             ...
			             1);
			if (!found) {
				report_error(UNKNOWN_SYMBOL, ident, "Unknown type member.");
				return_zone(FAIL);
			}

			bassert(found->kind == SCOPE_ENTRY_MEMBER);
			bassert(found->as.member && found->as.member->type);
			...
			// @Cleanup: This should be inside evaluator
			MIR_CEV_WRITE_AS(struct mir_type *, &member_ptr->base.value, found->as.member->type);
			return_zone(PASS);
		}
	}
	...
	const bool is_global   = !scope_is_local(member_ptr->base.node->owner_scope);
	const bool require_tmp = !mir_is_pointer_type(member_ptr->target_ptr->value.type);

	// struct type
	if (mir_is_composite_type(target_type)) {
		bassert(member_ptr->id_kind != MIR_INSTR_MEMBER_ID_BUILTIN);
		...
		if (require_tmp) {
			struct mir_instr *tmp_var = member_ptr->target_ptr->tmp_var;
			if (!tmp_var) {
				tmp_var = create_instr_decl_var_impl(ctx,
				                                     .name                  = unique_name(ctx, IMPL_TMP),
				...
				analyze_instr_rq(ctx, tmp_var);
				member_ptr->target_ptr->tmp_var = tmp_var;
			}
			tmp_var = ref_instr(create_instr_decl_direct_ref(ctx, NULL, tmp_var));
		...
		struct mir_member *member = NULL;

		if (member_ptr->id_kind == MIR_INSTR_MEMBER_ID_IDENT) {
			struct ast *ident = member_ptr->id.ident;

			struct id       *rid  = &ident->data.ident.id;
			...
			if (!found) {
				// Member not found!
				report_error(UNKNOWN_SYMBOL, ident, "Unknown structure member.");
				return_zone(FAIL);
			}
			...
			mir_members_t *members = target_type->data.strct.members;
			if (index >= sarrlenu(members)) {
				// Member not found!
				report_error(INVALID_EXPR, member_ptr->base.node, "Multi-value expression does not provide required value with index %u. Provided value count is %llu.", index, sarrlenu(members));
				return_zone(FAIL);
			}

			member = sarrpeek(members, index);
		}
		...
		member_ptr->scope_entry            = member->entry;
...
struct result analyze_instr_addrof(struct context *ctx, struct mir_instr_addrof *addrof) {
	...
	if (type->kind == MIR_TYPE_FN) {
		...
		if (fn->recipe_ext && fn->recipe_ext->flavor) {
			enum mir_fn_recipe_flavor flavor = fn->recipe_ext->flavor;
			if (isflag(flavor, MIR_FN_RECIPE_FLAVOR_POLY)) {
				report_error(EXPECTED_DECL,
				             addrof->base.node,
				             "Cannot take the address of polymorph function, its implementation may differ based on call side arguments, so the memory location may be ambiguous.");
				report_note(fn->decl_node, "Function declared here:");
			} else if (isflag(flavor, MIR_FN_RECIPE_FLAVOR_COMPTIME_ARGS)) {
				report_error(EXPECTED_DECL,
				             addrof->base.node,
				             "Cannot take the address of compile-time generated function, the implementation may differ based on call-side arguments. Compile-time generated function is function having at least one comptime argument.");
				report_note(fn->decl_node, "Function declared here:");
			} else {
				BL_UNREACHABLE;
			}
	...
	addrof->base.value.addr_mode   = MIR_VAM_LVALUE;
...
struct result analyze_instr_decl_ref(struct context *ctx, struct mir_instr_decl_ref *ref) {
	...
	case SCOPE_ENTRY_VARIANT: {
		struct mir_variant *variant = found->as.variant;
		bassert(variant);
	...
	case SCOPE_ENTRY_VAR: {
		struct mir_var *var = found->as.var;
		bassert(var);
		if (var->value.is_comptime && !isflag(var->iflags, MIR_VAR_ANALYZED)) {
			return_zone(POSTPONE);
		}
		...
		} else if (!isflag(var->iflags, MIR_VAR_GLOBAL) && out_of_function) {
	...
	case SCOPE_ENTRY_ARG: {
		struct mir_arg *arg = found->as.arg;
		bassert(arg);
...
struct result analyze_instr_fn_proto(struct context *ctx, struct mir_instr_fn_proto *fn_proto) {
	...
	if (fn->type->data.fn.ret_type->kind == MIR_TYPE_TYPE && fn->decl_node && !isflag(fn->flags, FLAG_COMPTIME)) {
		...
		report_error(INVALID_TYPE, fn_proto->base.node, "Invalid function return type 'type', mark the function as #comptime.");
		return_zone(FAIL);
	}
	...
	if (fn->id && !fn->linkage_name.len) { // Has ID and has no linkage name specified.
		...
		if (name_prefix.len) {
			fn->full_name = scprint(ctx->string_cache, "{str}.{str}", name_prefix, fn->id->str);
		} else {
	...
	if (isflag(fn->flags, FLAG_NO_CACHE) && !isflag(fn->flags, FLAG_COMPTIME)) {
		report_warning(fn_proto->base.node, "Directive '#nocache' ignored for regular runtime function.");
	}

	if (isflag(fn->flags, FLAG_EXTERN)) {
		bassert(!fn->dyncall_ext);
		fn->dyncall_ext = arena_alloc(&ctx->mir_arenas->fn_dyncall);

		// lookup external function exec handle
		fn->dyncall_ext->extern_entry = assembly_find_extern(ctx->assembly, fn->linkage_name);
		...
		bassert(!fn->dyncall_ext);
		fn->dyncall_ext               = arena_alloc(&ctx->mir_arenas->fn_dyncall);
		fn->dyncall_ext->extern_entry = assembly_find_extern(ctx->assembly, intrinsic_name);
		fn->is_fully_analyzed         = true;
	} else if (fn->recipe_ext) {
		// Nothing to do, function is just a recipe.
		bassert(fn->recipe_ext && "Missing generation recipe.");
	} else {
		// Add entry block of the function into analyze queue.
		struct mir_instr *entry_block = (struct mir_instr *)fn->entry_block;
		if (!entry_block) {
	...
	if (isflag(fn->flags, FLAG_EXPORT)) {
		...
		if (fn->recipe_ext) {
			...
			report_error(UNEXPECTED_DIRECTIVE, fn_proto->base.node, "Recipe function cannot be exported.");
			return_zone(FAIL);
		}
	}
...
struct result analyze_instr_fn_group(struct context *ctx, struct mir_instr_fn_group *group) {
	...
	struct result result           = PASS;
	mir_types_t  *variant_types    = arena_alloc(ctx->small_array_arena);
	mir_fns_t    *variant_fns      = arena_alloc(ctx->small_array_arena);
...
struct result analyze_instr_cond_br(struct context *ctx, struct mir_instr_cond_br *br) {
	...
	if (is_condition_comptime) {
		...
		// @Note 2026-01-17: Erase condition itself.
...
struct result analyze_instr_switch(struct context *ctx, struct mir_instr_switch *sw) {
	...
	if (expected_case_type->kind != MIR_TYPE_INT && expected_case_type->kind != MIR_TYPE_ENUM) {
		report_error(INVALID_TYPE, sw->value->node, "Invalid type of switch expression. Only integer types and enums can be used.");
		return_zone(FAIL);
	}
...
struct result analyze_instr_type_fn(struct context *ctx, struct mir_instr_type_fn *type_fn) {
	...
	if (type_fn->args) {
		...
		args = arena_alloc(ctx->small_array_arena);
		for (usize i = 0; i < argc; ++i) {
			bassert(sarrpeek(type_fn->args, i)->kind == MIR_INSTR_DECL_ARG);
...
struct result analyze_instr_decl_arg(struct context *ctx, struct mir_instr_decl_arg *decl) {
	...
	if (decl->type) {
		...
		if (arg->default_value->kind == MIR_INSTR_DECL_VAR) {
			...
			bassert(var);
			if (!isflag(var->iflags, MIR_VAR_ANALYZED)) return_zone(POSTPONE);
	...
	if (arg->type->kind == MIR_TYPE_TYPE && !isflag(arg->flags, FLAG_COMPTIME)) {
		report_error(INVALID_TYPE,
		             decl->base.node,
		             "Types can be passed only as comptime arguments into the functions. Use the '#comptime' directive for the argument '" STR_FMT "'.",
		             STR_ARG(arg->id->str));
		return_zone(FAIL);
...
struct result analyze_instr_type_struct(struct context *ctx, struct mir_instr_type_struct *type_struct) {
	...
	if (type_struct->members) {
		...
		struct mir_type              *member_type;
		members = arena_alloc(ctx->small_array_arena);
...
struct result analyze_instr_cond_insert(struct context *ctx, struct mir_instr_cond_insert *cond_insert) {
	zone();
	bassert(cond_insert->cond && cond_insert->then_block);
	bassert(cond_insert->cond->state == MIR_IS_COMPLETE);

	if (analyze_slot(ctx, analyze_slot_conf_default, &cond_insert->cond, ctx->builtin_types->t_bool) != ANALYZE_PASSED) {
		return_zone(FAIL);
	}

	const bool is_condition_comptime = mir_is_comptime(cond_insert->cond);
	if (!is_condition_comptime) {
		report_error(EXPECTED_COMPTIME, cond_insert->cond->node, "Static if expression is supposed to be known in compile-time.");
		return_zone(FAIL);
	}

	const bool  cond       = MIR_CEV_READ_AS(bool, &cond_insert->cond->value);
	struct ast *then_block = cond ? cond_insert->then_block : cond_insert->else_block;
	if (!then_block) return_zone(PASS);

	struct mir_insertion_context insertion_context = {0};
	insertion_begin(ctx, &insertion_context, &cond_insert->base, cond_insert->codegen);
	{
		if (then_block->kind == AST_BLOCK) {
			// Unroll the block. Note we don't create a new scope!
			for (usize i = 0; i < sarrlenu(then_block->data.block.nodes); ++i) {
				struct ast *tmp = sarrpeek(then_block->data.block.nodes, i);
				ast(ctx, tmp);
			}
		} else {
			ast(ctx, then_block);
...
struct result analyze_instr_defer(struct context *ctx, struct mir_instr_defer *defer) {
	zone();
	struct mir_fn *fn = mir_instr_owner_fn(&defer->base);
	bassert(fn && "Defer statement outside of function is not supported!");
	sarrput(&fn->defer_stack, defer->code);
	return_zone(PASS);
}

struct result analyze_instr_defer_insert(struct context *ctx, struct mir_instr_defer_insert *defer_insert) {
	zone();
	bassert(defer_insert->base.node);

	if (defer_insert->base.owner_block->is_unreachable) return_zone(PASS);

	struct scope *scope = defer_insert->base.node->owner_scope;
	bassert(scope);

	struct mir_fn *fn = mir_instr_owner_fn(&defer_insert->base);
	bassert(fn);
	...
	// clang-format off
	struct mir_insertion_context insertion_context = {0};
	insertion_begin(ctx, &insertion_context, &defer_insert->base, defer_insert->codegen); {
		while (scope_is_local(scope))  {
			for (s32 index = num; index-- > 0;) {
				struct ast *node = sarrpeek(&fn->defer_stack, index);
...
struct result analyze_instr_unop(struct context *ctx, struct mir_instr_unop *unop) {
	zone();
	if (mir_is_volatile_int_type(unop->expr)) {
		mir_set_volatile_int_type(&unop->base, unop->expr->value.type);
		return_zone(SKIP);
	}
...
#if BL_ASSERT_ENABLE
	if (isflag(var->iflags, MIR_VAR_GLOBAL_INLINE_INIT)) bassert(isflag(var->iflags, MIR_VAR_GLOBAL));
#endif

	if (isflag(var->iflags, MIR_VAR_GLOBAL) && !isflag(var->iflags, MIR_VAR_STRUCT_TYPEDEF) && !isflag(var->iflags, MIR_VAR_GLOBAL_INLINE_INIT)) {
...
#if BL_ASSERT_ENABLE
	if (isflag(var->iflags, MIR_VAR_GLOBAL_INLINE_INIT)) bassert(has_initializer);
#endif
...
static inline void begin_poly_replacement(struct context *ctx, mir_types_t *queue, struct mir_instr_call *call) {
	ctx->instantiate.queue     = queue;
	ctx->instantiate.index     = 0;
	ctx->instantiate.call      = call;
	ctx->instantiate.is_active = true;
}

static inline void clear_poly_replacement(struct context *ctx) {
	ctx->instantiate.queue = NULL;
	ctx->instantiate.index = 0;
}

static inline void end_poly_replacement(struct context *ctx) {
	bl_zeromem(&ctx->instantiate, sizeof(ctx->instantiate));
}
...
struct result analyze_call_slot(struct context *ctx, struct mir_instr_call *call, struct mir_arg *fn_arg) {
	...
	if (analyze_slot(ctx, analyze_slot_conf_call_arg_prescan, call_arg_instr_ref, fn_arg ? fn_arg->type : NULL) != ANALYZE_PASSED) {
		return_zone(FAIL);
	}
	...
	const bool comptime_required = isflag(fn_arg->flags, FLAG_COMPTIME) || mir_is_comptime(&call->base);

	// Check if the provided function argument is compile-time known in case it's required.
	if (comptime_required && !mir_is_comptime(*call_arg_instr_ref)) {
		report_error(EXPECTED_COMPTIME, (*call_arg_instr_ref)->node, "Function call argument is supposed to be compile-time known.");
...
static struct result analyze_call_stage_resolve_called_type(struct context *ctx, struct mir_instr_call *call);
...
static struct result analyze_call_stage_resolve_instance_type(struct context *ctx, struct mir_instr_call *call);
static struct result analyze_call_stage_instantiate(struct context *ctx, struct mir_instr_call *call);
...
Instantiation kicks in in case called function is polymorph or contains comptime arguments. In such case, user created function is
called "recipe". Recipe does not produce any real function body until it's called, only function prototype is created. Later when
we hit call instruction pointing to recipe, instantiation starts. In multiple steps below:

*/
static mir_call_analyze_stage_fn_t analyze_call_instantiate_pipeline[] = {
    &analyze_call_stage_prescan_arguments,     // Regular argument prescan.
    &analyze_call_stage_resolve_instance_type, // Here we take call argument list and recipe argument list and generate type resolver for real function signature. At this stage all polymorph "master" types are replaced with requested types.
    &analyze_call_stage_resolve_called_type,   // We have to wait for the type resolver, once it's analyzed and evaluated real type is set to call->called_fn_type.
    &analyze_call_stage_finalize,              // We use generated type to fully analyze all input args at this stage. Full analyze is required for later instance caching to work reliably. Also things like .MY_ENUM (where enum type is inferred from the argument slot) are properly analyzed.
    &analyze_call_stage_instantiate,           // At this stage we generate function signature identity (hash containing list of all replaced polymorph argument types and serialized values of comptime arguments). The identity hash is used to lookup in instance cache. New function is generated only in case it's not found in the cache.
    &analyze_call_stage_resolve_called_object, // Final step here is to wait for replaced callee analyze.
    NULL,
};
...
struct result analyze_call_stage_resolve_called_object(struct context *ctx, struct mir_instr_call *call) {
	...
	if (call->callee->state != MIR_IS_COMPLETE) {
		// Case for directly called anonymous functions `fn (){ ... }()`.
...
struct result analyze_call_stage_resolve_called_type(struct context *ctx, struct mir_instr_call *call) {
	zone();
	bassert(call->instantiation.type_resolver);
	struct result result = analyze_resolve_type(ctx, call->instantiation.type_resolver, &call->called_fn_type);
	if (result.state != ANALYZE_PASSED) return_zone(result);

	bassert(call->called_fn_type);

	// Validate type.
	if (call->called_fn_type->kind != MIR_TYPE_FN) {
...
struct result analyze_call_stage_validate_called_object(struct context *ctx, struct mir_instr_call *call) {
	...
	if (fn_type->kind == MIR_TYPE_FN) {
		if (mir_is_comptime(call->callee)) {
			call->called_fn = MIR_CEV_READ_AS(struct mir_fn *, &call->callee->value);
			bmagic_assert(call->called_fn);

			if (isflag(call->called_fn->flags, FLAG_COMPTIME)) call->base.value.is_comptime = true;
			if (call->base.value.is_comptime && call->catch_block) {
				report_error(UNEXPECTED_CATCH, call->catch_block, "Catch expression cannot be used on comptime function. Call result will be converted to compile-time known constant.");
				return_zone(FAIL);
			}

			if (call->called_fn->recipe_ext) {
				...
				//
				// We may have to find a better solution for this, but currently I have no other ideas...
				...
				for (usize index = 0; index < sarrlenu(call->args); ++index) {
					...
					if (call_arg_instr->value.type && call_arg_instr->value.type->kind == MIR_TYPE_PLACEHOLDER) {
				...
				if (is_called_with_placeholder) {
					...
					call->analyze_pipeline = analyze_call_instantiate_pipeline;
				}
			} else {

				// @Note 2025-12-08: In case of inferred compound or enum types, we have to provide argument types as soon as possible.
				//                   Note that the same analyzer steps are visited again later when call analyze is finalized, so there
				//                   is some room for optimization.
				for (usize index = 0; index < sarrlenu(call->args); ++index) {
					struct mir_instr **call_arg_instr_ref = &sarrpeek(call->args, index);
	...
	} else if (fn_type->kind == MIR_TYPE_FN_GROUP) {
...
struct result analyze_call_stage_resolve_instance_type(struct context *ctx, struct mir_instr_call *call) {
	zone();
	bcalled_once_assert(call, generate_proto);

	// We're calling polymorph or mixed function recipe, so we have to generate its
	// implementation first!
	bassert(call->called_fn);
	runtime_measure_begin(generated);

	struct result result = PASS;

	struct mir_fn *recipe_fn = call->called_fn;
	bmagic_assert(recipe_fn);
	bassert(recipe_fn->recipe_ext);

	struct mir_fn_recipe *recipe = recipe_fn->recipe_ext;
	bmagic_assert(recipe);

	// --------------------------------------------------------------------------------
	// PHASE 1: Resolve polymorph replacement.
	// --------------------------------------------------------------------------------
	const bool is_polymorph = isflag(recipe->flavor, MIR_FN_RECIPE_FLAVOR_POLY);
	if (is_polymorph) {
		...
		call->instantiation.type_replacement_queue = arena_alloc(ctx->small_array_arena);
		mir_types_t *queue                         = call->instantiation.type_replacement_queue;
		...
		if (debug_replacement_str.len) {
			// @Performance 2026-09-04: We actually need debug replacement only in case of user program error, also we generate it for every call and use eventually only one in case the function call is found in cache later in analyze_call_stage_instantiate.
			call->instantiation.debug_replacement = scdup2(ctx->string_cache, debug_replacement_str);
		}
		put_tmp_str(debug_replacement_str);
	}

	// --------------------------------------------------------------------------------
	// PHASE 2: Generate function signature.
	// --------------------------------------------------------------------------------

	const s32 prev_errorc = builder.errorc;

	// Prepare global state for the function generation.
	begin_poly_replacement(ctx, call->instantiation.type_replacement_queue, isflag(recipe->flavor, MIR_FN_RECIPE_FLAVOR_COMPTIME_ARGS) ? call : NULL);
	ctx->codegen->current_scope_layer        = ++recipe->scope_layer;
	const bool prev_is_inside_fn_declaration = ctx->codegen->is_inside_fn_declaration;
	ctx->codegen->is_inside_fn_declaration   = true;

	call->instantiation.type_resolver = ast_create_type_resolver_call(ctx, recipe->ast_lit_fn->data.expr_fn.type);

	// Resume state.
	end_poly_replacement(ctx);
	ctx->codegen->current_scope_layer      = SCOPE_DEFAULT_LAYER;
	ctx->codegen->is_inside_fn_declaration = prev_is_inside_fn_declaration;

	// Handle invalid AST generation.
	if (builder.errorc != prev_errorc) {
		result = FAIL;
		goto DONE;
	}
...
struct result analyze_call_stage_instantiate(struct context *ctx, struct mir_instr_call *call) {
#define DEBUG_CACHE 0

	zone();
	bcalled_once_assert(call, generate_body);
	struct mir_type *fn_type = call->called_fn_type;
	mir_types_t     *queue   = call->instantiation.type_replacement_queue;
	...
	str_buf_t identity = get_tmp_str();

	for (usize index = 0; index < sarrlenu(queue); ++index) {
		struct mir_type *replacement_type = sarrpeek(queue, index);
		str_buf_append_fmt(&identity, "{u32};", replacement_type->id.hash);
	}

	mir_args_t *args              = fn_type->data.fn.args;
	const usize type_argc         = sarrlenu(args);
	const usize call_argc         = sarrlenu(call->args);
	usize       args_comptime_num = 0;
	bassert(type_argc == call_argc);
	for (usize index = 0; index < MIN(type_argc, call_argc); ++index) {
		struct mir_instr *instr = sarrpeek(call->args, index);
		struct mir_arg   *arg   = sarrpeek(args, index);
		if (isflag(arg->flags, FLAG_COMPTIME)) {
			bassert(instr->state == MIR_IS_COMPLETE);
			vm_serialize_const_expr_value(&instr->value, &identity);
			str_buf_append(&identity, cstr(";"));
			++args_comptime_num;
		}
	}
	...
	const hash_t identity_hash = strhash(identity);
	const s32    index         = tbl_lookup_index_with_key(recipe->entries, identity_hash, str_buf_view(identity));
	if (index == -1) {
#if DEBUG_CACHE
		blog("NEW[%llu]:    " STR_FMT " >> " STR_FMT, call->base.id, STR_ARG(call->called_fn->linkage_name), STR_ARG(identity));
#endif

		// Create name for generated function
		const str_t original_fn_name = recipe_fn->id ? recipe_fn->id->str : IMPL_FN_NAME;

		struct mir_instr_call *generate_call = NULL;
		if (isflag(recipe->flavor, MIR_FN_RECIPE_FLAVOR_COMPTIME_ARGS)) {
			// In case the function is mixed (has comptime arguments) we must provide them.
			// i.e.: fn (v: s32 #comptime, arr: [v]s32)
			// 2025-12-16: We have to duplicate here in case the call is comptime, the call instruction is
			//             later mutated to constant and all call related information is lost. This mutation
			//             happens before new body is processed and all comptime arguments are replaced.
			generate_call = mir_is_comptime(&call->base) ? (struct mir_instr_call *)duplicate_instr(ctx, &call->base) : call;
		}

		// Prepare global state for the function generation.
		begin_poly_replacement(ctx, call->instantiation.type_replacement_queue, generate_call);
		ctx->codegen->current_scope_layer = ++recipe->scope_layer;

		const s32 prev_errorc = builder.errorc;
		// Generate new function.
		struct mir_instr *instr_fn_proto = ast_expr_lit_fn(ctx,
		                                                   .lit_fn                = recipe->ast_lit_fn,
		                                                   .decl_node             = recipe_fn->decl_node,
		                                                   .explicit_linkage_name = unique_name(ctx, original_fn_name),
		                                                   .is_global             = recipe_fn->is_global,
		                                                   .flags                 = recipe_fn->flags);
		...
		bassert(!replacement_fn->instance_ext);
		replacement_fn->instance_ext = arena_alloc(&ctx->mir_arenas->fn_instance);

		replacement_fn->instance_ext->first_call_node         = call->base.node;
		replacement_fn->instance_ext->debug_replacement_types = call->instantiation.debug_replacement;
		if (!isflag(replacement_fn->flags, FLAG_NO_CACHE) && args_comptime_num == type_argc && recipe->flavor == MIR_FN_RECIPE_FLAVOR_COMPTIME_ARGS && isflag(replacement_fn->flags, FLAG_COMPTIME)) {
			replacement_fn->instance_ext->is_exec_result_cache_enabled = true;
		}

		struct recipe_entry entry = (struct recipe_entry){
		    .hash        = identity_hash,
		    .key         = scdup2(ctx->string_cache, identity),
...
#if DEBUG_CACHE
		blog("CACHED[%llu]: " STR_FMT " >> " STR_FMT, call->base.id, STR_ARG(call->called_fn->linkage_name), STR_ARG(identity));
#endif
		replacement_fn = recipe->entries[index].replacement;
	}

DONE:
	put_tmp_str(identity);
...
struct result analyze_call_stage_prescan_arguments(struct context *ctx, struct mir_instr_call *call) {
	...
	const bool is_generated = call->called_fn && call->called_fn->recipe_ext;
	...
	for (usize index = 0; index < MAX(func_argc, call_argc); ++index) {
		...
		if (fn_arg->type->kind == MIR_TYPE_VARGS) {
			bassert(!isflag(fn_arg->flags, FLAG_COMPTIME) && "Comptime vargs are not supported!");
...
INVALID_ARGS:
	...
	if (call->called_fn && call->called_fn->decl_node) {
		report_note(call->called_fn->decl_node, "Function declared here:");
	}
	return_zone(FAIL);
}
...
struct result analyze_call_stage_finalize(struct context *ctx, struct mir_instr_call *call) {
	...
	if (call->callee->kind == MIR_INSTR_FN_PROTO) {
		...
		++call->called_fn->ref_count;
	}

	// @Note 2025-12-02: The called_function is optional, might not be set for example when function is called via pointer,
	//                   we might reinvestigate this; but it will be problematic, function pointers are not known in compile
	//                   time.
	const enum ast_flags called_fn_flags = call->called_fn ? call->called_fn->flags : 0;

	struct mir_type *fn_type = call->called_fn_type;
	bassert(fn_type && fn_type->kind == MIR_TYPE_FN);
	bassert(!fn_type->data.fn.is_polymorph);
	...
	for (usize index = 0; index < MAX(func_argc, call_argc); ++index) {
		...
		if (isflag(fn_arg->flags, FLAG_COMPTIME) && !isflag(called_fn_flags, FLAG_EXTERN)) {
...
struct result analyze_instr_call(struct context *ctx, struct mir_instr_call *call) {
	...
	if (call->called_fn) {
		struct mir_fn *fn = call->called_fn;
...
struct result analyze_instr_block(struct context *ctx, struct mir_instr_block *block) {
	...
	if (!is_block_terminated(block)) {
		...
		if (fn->type->data.fn.ret_type->kind == MIR_TYPE_VOID) {
			...
			// @Note 2026-01-26: This is related to the fact that we might have defer insert instruction already in place
			//                   in case it was introduced explicitly by user-defined lexical scope. Thus we check if it's
	...
	if (!block->entry_instr) {
		analyze_instr_block_finalize(ctx, block);
	}

	return_zone(PASS);
}

void analyze_instr_block_finalize(struct context *ctx, struct mir_instr_block *block) {
	bassert(block);
	struct mir_fn *fn = block->owner_fn;
	if (!fn) { // block in global scope
		return;
	...
	report_error(MISSING_RETURN, fn->decl_node, "Not every path inside the function return value.");
}
...
static void insert_and_analyze_tmp_var(struct context *ctx, struct mir_instr **input, const str_t name, const bool is_comptime) {
...
enum stage_state analyze_stage_toslice(struct context *ctx, struct mir_instr *parent_instr, struct mir_instr **input, struct mir_type *slot_type, bool is_initializer) {
	...
	if (!is_simple_cast) {
		...
		// variable.
		insert_and_analyze_tmp_var(ctx, input, IMPL_TOSLICE_TMP, false);
	}
...
enum stage_state analyze_stage_arrtoslice(struct context *ctx, struct mir_instr *parent_instr, struct mir_instr **input, struct mir_type *slot_type, bool is_initializer) {
	...
	struct mir_type *from_type   = (*input)->value.type;
	const bool       is_comptime = (*input)->value.is_comptime;
	bassert(from_type);
	...
	if (!is_reference) {
		insert_and_analyze_tmp_var(ctx, input, IMPL_TOSLICE_TMP, is_comptime);
	}

	// Optional node to provide debug locations for generated instructions.
	struct ast *ast_node = (*input)->node;

	// Build array pointer
	struct mir_instr *instr_ptr = create_instr_member_ptr(ctx, .node = ast_node, .target_ptr = *input, .builtin_id = BUILTIN_ID_ARR_PTR);
	insert_instr_after(*input, instr_ptr);
	*input = instr_ptr;
	analyze_instr_rq(ctx, instr_ptr);

	// Build array len constant
	struct mir_instr *instr_len = create_instr_const_int(ctx, ast_node, ctx->builtin_types->t_s64, len, false);
	...
	*input = instr_len;
	...
	struct mir_instr *compound                        = create_instr_compound_impl(ctx, ast_node, slot_type, values);
...
static struct mir_type *get_preffered_type_for_volatile_propagation(struct context *ctx, struct mir_type *current, struct mir_type *volatile_candidate) {
	bassert(volatile_candidate && volatile_candidate->kind == MIR_TYPE_INT);
	if (!current) return volatile_candidate;
	bassert(volatile_candidate->kind == current->kind);
	if (volatile_candidate->store_size_bytes > current->store_size_bytes) {
		return volatile_candidate;
	}
...
static struct result analyze_propagate_volatile_type(struct context *ctx, struct mir_instr *root_instr, struct mir_type *type) {
	...
	bassert(!type || type->kind == MIR_TYPE_INT);
	struct mir_type *target_type = type;
	...
	while (index < sarrlen(&queue)) {
		...
		bassert(mir_is_volatile_int_type(instr));

		switch (instr->kind) {
		case MIR_INSTR_CONST:
			if (!type) target_type = get_preffered_type_for_volatile_propagation(ctx, target_type, instr->value.type);
			break;
		case MIR_INSTR_UNOP: {
	...
	while (index >= 0 && result.state == ANALYZE_PASSED) {
		...
		instr->value.is_type_volatile = false;

		switch (instr->kind) {
		case MIR_INSTR_CONST: {
			const enum mir_cast_op op = get_cast_op(instr->value.type, target_type);
			if (op == MIR_CAST_NONE) {
				instr->value.type = target_type;
			} else if (op != MIR_CAST_INVALID) {
				struct mir_const_expr_value *value = &instr->value;
				vm_value_t                   tmp   = {0};
				vm_do_cast((vm_stack_ptr_t)&tmp[0], value->data, target_type, value->type, op);
	...
	if (!type && root_type->kind == MIR_TYPE_INT && result.state == ANALYZE_PASSED) {
		...
		if (root_type->data.integer.is_signed) {
			...
			case 4: {
				const s32 n = MIR_CEV_READ_AS(s32, &root_instr->value);
				if (n == SHRT_MIN) root_instr->value.type = ctx->builtin_types->t_s8;
				break;
			}
			case 8: {
				const s64 n = MIR_CEV_READ_AS(s64, &root_instr->value);
				if (n == INT_MIN) root_instr->value.type = ctx->builtin_types->t_s32;
				break;
			}
			default:
				break;
...
enum stage_state analyze_stage_propagate_volatile_type(struct context *ctx, struct mir_instr *parent_instr, struct mir_instr **input, struct mir_type *slot_type, bool is_initializer) {
	if (!mir_is_volatile_int_type(*input)) return ANALYZE_STAGE_CONTINUE;
	struct mir_type *propagate_type = NULL;
	// @Note 2025-11-24: Try to find best base type for the volatile expression.
	if (slot_type) {
		switch (slot_type->kind) {
		case MIR_TYPE_INT:
			propagate_type = slot_type;
			break;
		case MIR_TYPE_PTR:
			propagate_type = ctx->builtin_types->t_s64;
			break;
		case MIR_TYPE_ENUM:
			propagate_type = slot_type->data.enm.base_type;
			bassert(propagate_type);
			break;
		default:
			break;
		}
...
struct result analyze_instr(struct context *ctx, struct mir_instr *instr) {
	...
	if ((*analyze_state) == MIR_IS_PENDING) {
		...
		case MIR_INSTR_COND_BR:
			...
			break;
		case MIR_INSTR_DEFER:
			analyze_instr_defer(ctx, (struct mir_instr_defer *)instr);
			break;
		case MIR_INSTR_DEFER_INSERT:
			analyze_instr_defer_insert(ctx, (struct mir_instr_defer_insert *)instr);
			break;
		case MIR_INSTR_COND_INSERT:
			state = analyze_instr_cond_insert(ctx, (struct mir_instr_cond_insert *)instr);
			break;
		case MIR_INSTR_UNOP:
...
#if BL_ASSERT_ENABLE
			bassert(is_one_of(instr->kind, MIR_INSTR_COMPOUND, MIR_INSTR_BINOP, MIR_INSTR_UNOP, MIR_INSTR_DECL_REF) && "ANALYZE_SKIP is supported only for limited set of instructions!");
	...
	if ((*analyze_state) == MIR_IS_ANALYZED) {
		...
		if (instr->kind == MIR_INSTR_CAST && ((struct mir_instr_cast *)instr)->auto_cast) {
			...
			return_zone(state);
		} else if (instr->kind == MIR_INSTR_COND_INSERT) {
			(*analyze_state) = MIR_IS_COMPLETE;
			return_zone(state);
		}
...
static inline struct mir_instr *analyze_get_next_instr(struct context *ctx, struct mir_instr *curr_instr, usize *index, usize *stack_index, bool skip) {
	if (!skip) {
		struct mir_instr *next = analyze_try_get_next(curr_instr);
		if (next) return next;
	}

	if (*index >= arrlenu(ctx->analyze->stack[*stack_index])) {
		arrsetlen(ctx->analyze->stack[*stack_index], 0);
		*index       = 0;
		*stack_index = analyze_swap(ctx);
		if (arrlenu(ctx->analyze->stack[*stack_index]) == 0)
			return NULL;
	}

	return ctx->analyze->stack[*stack_index][(*index)++];
}

static inline void analyze_enque_waiting(struct context *ctx, struct mir_instr *curr_instr, struct result result) {
	instrs_t    *wait_queue = NULL;
	...
	if (index == -1) {
		...
		index = (s32)tbl_len(ctx->analyze->waiting) - 1; // New item is the last in the array.
		bassert(index >= -1);

		wait_queue = &ctx->analyze->waiting[index].value;
	} else {
		wait_queue = &ctx->analyze->waiting[index].value;
	}

	bassert(wait_queue);
	sarrput(wait_queue, curr_instr);
}

static inline bool can_erase_instr(struct mir_instr *instr) {
	return instr->state == MIR_IS_COMPLETE && instr->ref_count == 0;
}
...
void analyze(struct context *ctx) {
	...
	struct result     result;
	usize             pending_count = 0, index = 0, stack_index = analyze_swap(ctx);
	struct mir_instr *curr_instr = NULL, *prev_instr = NULL;
	bool              skip = false;

	while (true) {

		// Remove unused instructions here!
		if (prev_instr && can_erase_instr(prev_instr)) {
			erase_instr_tree(prev_instr, false, false);
		}

		prev_instr = curr_instr;
		curr_instr = analyze_get_next_instr(ctx, curr_instr, &index, &stack_index, skip);
		skip       = false;

		if (!curr_instr) break;
		bmagic_assert(curr_instr);

		result = analyze_instr(ctx, curr_instr);
		if (is_last_instr_in_block(curr_instr)) {
			analyze_instr_block_finalize(ctx, curr_instr->owner_block);
		}
		...
		case ANALYZE_POSTPONE:
			...
			if (pending_count++ <= analyze_pending_count(ctx)) analyze_schedule(ctx, curr_instr);
			skip = true;
			break;

		case ANALYZE_WAIT:
			analyze_enque_waiting(ctx, curr_instr, result);
			pending_count = 0;
			skip          = true;
			break;
		}
...
static bool analyze_detect_cyclic_deps(struct context *ctx, struct mir_instr_decl_ref *root_ref) {
	...
	while (arrlen(queue)) {
		...
		struct ast *ast_names = parent_decl->data.decl.name;
		bassert(ast_names->kind == AST_LIST);

		ast_nodes_small_t *names = &ast_names->data.list.items;

		for (usize i = 0; i < sarrlenu(names); ++i) {
			struct ast *name = sarrpeek(names, i);
			bassert(name->kind == AST_IDENT);

			if (name == root_ref->scope_entry->node) {
				blog("Loop detected!");
				result = true;
				break;
			}

			const s32 waiting_group_index = tbl_lookup_index(ctx->analyze->waiting, name->data.ident.id.hash);
...
struct mir_instr *ast_stmt_if(struct context *ctx, struct ast *stmt_if) {
	...
	if (is_static) {
		return append_instr_cond_insert(ctx, stmt_if, cond, ast_then, ast_else);
	}
	...
	{ // then block
		then_block = append_block(ctx, ast_then, fn, cstr("if_then"), is_unreachable);
	...
	if (ast_else) { // else block
		else_block = append_block(ctx, ast_else, fn, cstr("if_else"), is_unreachable);
	...
	// continue block
	continue_block = append_block(ctx, stmt_if, fn, cstr("if_continue"), is_unreachable);
...
void ast_stmt_loop(struct context *ctx, struct ast *loop) {
	...
	struct mir_instr_block *increment_block     = ast_increment ? append_block(ctx, NULL, fn, cstr("loop_increment"), is_unreachable) : NULL;
	struct mir_instr_block *decide_block        = append_block(ctx, NULL, fn, cstr("loop_decide"), is_unreachable);
	struct mir_instr_block *body_block          = append_block(ctx, NULL, fn, cstr("loop_body"), is_unreachable);
...
void ast_stmt_switch(struct context *ctx, struct ast *stmt_switch) {
	...
	mir_switch_cases_t *cases = arena_alloc(ctx->small_array_arena);
	...
	struct mir_instr_block *cont_block           = append_block(ctx, NULL, fn, cstr("switch_continue"), is_unreachable);
	...
	for (usize i = sarrlenu(ast_cases); i-- > 0;) {
		...
		if (ast_case->data.stmt_case.block) {
			case_block                 = append_block(ctx, NULL, fn, is_default ? cstr("switch_default") : cstr("switch_case"), is_unreachable);
...
static inline void ast_append_compound_assignment_operation(struct context *ctx, struct ast *assign, enum binop_kind op) {
	...
	bassert(ast_lhs->kind == AST_LIST);
	bassert(sarrlen(&ast_lhs->data.list.items) == 1);
	ast_lhs = sarrpeek(&ast_lhs->data.list.items, 0);
...
void ast_stmt_assign(struct context *ctx, struct ast *assign) {
	...
	case ASSIGN: {
		...
		for (u32 index = 0; index < sarrlenu(lhs_exprs); ++index) {
			ast_lhs = sarrpeek(lhs_exprs, index);

			struct ast *ident = ast_lhs->kind == AST_REF ? ast_lhs->data.ref.ident : NULL;
			if (ident && is_ignored_id(&ident->data.ident.id)) {
				++ignored_num;
				continue;
			}

			struct mir_instr *current_rhs = rhs;
			if (use_unroll) {
				current_rhs = append_instr_unroll(ctx, ast_lhs, current_rhs, index, true);
			}
			...
			append_instr_store(ctx, assign, current_rhs, lhs);
		}

		if (ignored_num == sarrlenu(lhs_exprs)) {
			report_warning(assign, "All values ignored, assignment might be removed.");
		}
...
struct mir_instr *ast_stmt_defer(struct context *ctx, struct ast *defer) {
	return append_instr_defer(ctx, defer, defer->data.stmt_defer.expr);
}
...
struct mir_instr *ast_expr_lit_int(struct context *ctx, struct ast *expr) {
	...
	return append_instr_const_int(ctx, expr, type, val, true);
}
...
struct mir_instr *ast_expr_call(struct context *ctx, struct ast *call) {
	...
	mir_instrs_t *args = arena_alloc(ctx->small_array_arena);
	...
	return append_instr_call(ctx,
	                         .node             = call,
	                         .callee           = ast(ctx, ast_callee),
	                         .args             = args,
	                         .is_comptime      = call->data.expr_call.is_comptime,
	                         .is_inside_recipe = ctx->codegen->is_inside_recipe);
}
...
struct mir_instr *ast_expr_catch(struct context *ctx, struct ast *catch) {
	...
	bassert(call && call->base.kind == MIR_INSTR_CALL);
	call->catch_block = catch;

	struct mir_instr_call *prev_catched_call = ctx->codegen->current_catched_call;
	ctx->codegen->current_catched_call       = call;

	// Optional custom condition.
	struct mir_instr *cond = append_instr_unroll(ctx, catch, &call->base, UNROLL_LAST_INDEX, true);

	struct mir_instr_block *catch_block    = append_block(ctx, NULL, fn, cstr("catch"), is_unreachable);
	struct mir_instr_block *continue_block = append_block(ctx, NULL, fn, cstr("catch_continue"), is_unreachable);
	...
	set_current_block(ctx, continue_block);
	ctx->codegen->current_catched_call = prev_catched_call;

	return &call->base;
}

struct mir_instr *ast_expr_err(struct context *ctx, struct ast *err) {
	struct mir_instr_call *catched_call = ctx->codegen->current_catched_call;
	if (!catched_call) {
	...
	return append_instr_unroll(ctx, err, &catched_call->base, UNROLL_LAST_INDEX, true);
}
...
struct mir_instr *_ast_expr_lit_fn(struct context *ctx, ast_expr_lit_fn_args_t *args) {
	struct ast *ast_body      = args->lit_fn->data.expr_fn.block;
	struct ast *ast_type      = args->lit_fn->data.expr_fn.type;
	struct ast *ast_enable_if = args->lit_fn->data.expr_fn.enable_if;

	struct mir_instr_block *prev_block = ast_current_block(ctx);
	struct ast             *err_node   = args->decl_node ? args->decl_node : args->lit_fn;

	// --------------------------------------------------------------------------------
	// PHASE 1: Function prototype
	// --------------------------------------------------------------------------------

	// Convert type flavor to function flavor here.
	enum mir_fn_recipe_flavor recipe_flavor = MIR_FN_RECIPE_FLAVOR_NONE;
	if (!ctx->instantiate.is_active) {
		const enum ast_type_fn_flavor function_type_flavor = ast_type->data.type_fn.flavor;
		if (isflag(function_type_flavor, AST_TYPE_FN_FLAVOR_POLYMORPH)) {
			setflag(recipe_flavor, MIR_FN_RECIPE_FLAVOR_POLY);
		}
		// @Note 2025-12-01: Disable function generation for extern mixed functions, such a function has no body
		//                   thus there is nothing to be generated. We use this for comptime compiler builtin functions
		//                   mainly taking 'type' in arguments.
		if (isflag(function_type_flavor, AST_TYPE_FN_FLAVOR_MIXED) && !isflag(args->flags, FLAG_EXTERN)) {
			setflag(recipe_flavor, MIR_FN_RECIPE_FLAVOR_COMPTIME_ARGS);
		}
	}

	const bool is_recipe = recipe_flavor != 0;
	...
	struct id *id = (args->decl_node && args->decl_node->kind == AST_IDENT) ? &args->decl_node->data.ident.id : NULL;
	bassert(ast_type->kind == AST_TYPE_FN);

	const bool prev_is_inside_fn_declaration = ctx->codegen->is_inside_fn_declaration;
	ctx->codegen->is_inside_fn_declaration   = true;
	ctx->codegen->is_inside_recipe           = is_recipe;

	struct mir_instr_fn_proto *fn_proto = (struct mir_instr_fn_proto *)append_instr_fn_proto(ctx, args->lit_fn, NULL, NULL, true);

	// Generate type resolver for function type.
	fn_proto->type = ast_create_type_resolver_call(ctx, ast_type);
	...
	ctx->codegen->is_inside_recipe         = false;
	ctx->codegen->is_inside_fn_declaration = prev_is_inside_fn_declaration;

	struct mir_fn *fn = create_fn(ctx,
	                              .decl_node    = args->decl_node ? args->decl_node : args->lit_fn,
	                              .id           = id,
	                              .linkage_name = args->explicit_linkage_name,
	                              .flags        = args->flags,
	                              .prototype    = fn_proto,
	                              .is_global    = args->is_global,
	                              .builtin_id   = args->builtin_id);
	...
	if (is_recipe) {
		// Add recipe extension to the function.
		fn->recipe_ext = create_fn_recipe(ctx, args->lit_fn, recipe_flavor);
	}

	MIR_CEV_WRITE_AS(struct mir_fn *, &fn_proto->base.value, fn);

	if (isflag(fn->flags, FLAG_OBSOLETE)) {
		struct ast *ast_optional_message = args->lit_fn->data.expr_fn.obsolete_warning_message;
	...
	if ((isflag(fn->flags, FLAG_EXTERN) || isflag(fn->flags, FLAG_INTRINSIC) || isflag(fn->flags, FLAG_EXPORT)) && is_recipe) {
		report_error(INVALID_CALL_CONVENTION, err_node, "External, exported or intrinsic functions must follow C call conventions and cannot be polymorph or having compile-time known arguments (those features are BL specific).");
		goto FINISH;
	}
	...
	if (isflag(args->flags, FLAG_EXTERN) || isflag(args->flags, FLAG_INTRINSIC)) {
		if (ast_body) {
			report_error(UNEXPECTED_FUNCTION_BODY, err_node, "Unexpected body for %s function.", isflag(args->flags, FLAG_EXTERN) ? "external" : "intrinsic");
		}
		goto FINISH;
	}

	if (!ast_body) {
		report_error(EXPECTED_BODY, err_node, "Missing function body.");
		goto FINISH;
	}

	if (is_recipe) {
		// Skipping here, recipe function is actually supposed to have a body, we just don't generate it until recipe is instantiated.
		goto FINISH;
	}

	// Set body scope for DI.
	bassert(ast_body->owner_scope && ast_body->owner_scope->kind == SCOPE_FN_BODY);
	fn->body_scope = ast_body->owner_scope;

	// create block for initialization locals and arguments
	struct mir_instr_block *entry_block = append_block(ctx, ast_body, fn, cstr("entry"), false);
	entry_block->base.ref_count         = MIR_NO_REF_COUNTING;
	...
	struct mir_instr_block *exit_block = append_block(ctx, ast_body, fn, cstr("exit"), false);
	fn->exit_block                     = (struct mir_instr_block *)ref_instr(&exit_block->base); // Exit block is always referenced

	if (ast_type->data.type_fn.ret_type) {
		set_current_block(ctx, entry_block);
		...
		struct mir_instr *ret_init = append_instr_decl_direct_ref(ctx, ast_body, fn->ret_tmp);
		append_instr_ret(ctx, ast_body, ret_init, false);
	} else {
		set_current_block(ctx, exit_block);
		append_instr_ret(ctx, ast_body, NULL, false);
	}
	set_current_block(ctx, entry_block);

	// build MIR for fn arguments
	ast_nodes_t *ast_args = ast_type->data.type_fn.args;
	if (ast_args) {
		struct ast *ast_arg;
...
struct mir_instr *ast_expr_lit_fn_group(struct context *ctx, struct ast *group) {
	...
	for (usize i = 0; i < sarrlenu(ast_variants); ++i) {
		...
		if (it->kind == AST_EXPR_LIT_FN) {
			variant = ast_expr_lit_fn(ctx, .lit_fn = it, .is_global = true);
		} else {
			variant = ast(ctx, it);
		}
...
struct mir_instr *ast_expr_binop(struct context *ctx, struct ast *binop) {
	...
	case BINOP_LOGIC_OR: {
		...
		// rhs
		struct mir_instr_block *rhs_block = append_block(ctx, NULL, fn, cstr("rhs_block"), is_unreachable);
		set_current_block(ctx, rhs_block);
		struct mir_instr       *rhs                = ast(ctx, ast_rhs);
		struct mir_instr_block *rhs_continue_block = ast_current_block(ctx);

		struct mir_instr_block *continue_block = append_block(ctx, NULL, fn, cstr("continue_block"), is_unreachable);
...
void report_poly(struct mir_instr *instr) {
	...
	if (!owner_fn) return;
	if (!owner_fn->instance_ext) return;
	if (!owner_fn->instance_ext->first_call_node) return;
	if (!owner_fn->instance_ext->first_call_node->location) return;
	const str_t debug_replacement = owner_fn->instance_ext->debug_replacement_types;
	if (debug_replacement.len) {
	...
	builder_msg(MSG_ERR_NOTE, 0, owner_fn->instance_ext->first_call_node->location, CARET_WORD, "First called here:");
}

// Helper for function declaration generation.
static void ast_decl_fn(struct context *ctx, struct ast *ast_fn) {
	struct ast *ast_names = ast_fn->data.decl.name;
	struct ast *ast_type  = ast_fn->data.decl.type;
	struct ast *ast_value = ast_fn->data.decl_entity.value;

	bassert(ast_names->kind == AST_LIST);
	ast_nodes_small_t *names = &ast_names->data.list.items;
	bassert(sarrlen(names) > 0);

	bassert(ast_value->kind == AST_EXPR_LIT_FN);

	struct ast *ast_name = sarrpeek(names, 0);
	bassert(ast_name->kind == AST_IDENT);

	if (sarrlen(names) > 1) {
		report_error(INVALID_NAME, ast_name, "Functions cannot be multi-declared.");
	}
	...
	struct mir_instr *value = ast_expr_lit_fn(ctx,
	                                          .lit_fn                = ast_value,
	                                          .decl_node             = ast_name,
	                                          .explicit_linkage_name = optional_explicit_linkage_name,
	                                          .is_global             = is_global,
	                                          .flags                 = flags,
...
static void ast_decl_var_local(struct context *ctx, struct ast *ast_local) {
	struct ast *ast_names = ast_local->data.decl.name;
	...
	struct scope     *scope       = ast_names->owner_scope;
	...
	const bool        force_tmp   = ast_value && ast_value->kind == AST_EXPR_CATCH;
	const bool        use_unroll  = ast_value && (ast_value->kind == AST_EXPR_CALL || ast_value->kind == AST_EXPR_CATCH);

	ast_nodes_small_t *names = &ast_names->data.list.items;

	struct mir_instr *prev = NULL;

	for (usize i = 0; i < sarrlenu(names); ++i) {
		struct ast *ast_name = sarrpeek(names, i);
		bassert(ast_name->kind == AST_IDENT);

		struct mir_instr *init = value;

		enum builtin_id_kind builtin_id = BUILTIN_ID_NONE;
		if (is_compiler) builtin_id = check_symbol_marked_compiler(ctx, ast_name);

		if (use_unroll) {
			init = append_instr_unroll(ctx, ast_name, init, (u32)i, force_tmp);
		} else if (prev) {
			bassert(!force_tmp);
			// Generated as cascade a, b, c :: 10 -> a :: 10; b :: a; c :: b;
			init = append_instr_decl_direct_ref(ctx, ast_name, prev);
		}
		...
		((struct mir_instr_decl_var *)var)->var->entry = register_symbol(ctx, ast_name, id, scope, is_compiler);
		prev                                           = var;
	}
}

static void ast_decl_var_struct(struct context *ctx, struct ast *ast_global) {
	struct ast *ast_names = ast_global->data.decl.name;
	...
	if (sarrlen(names) > 1) {
		report_error(INVALID_NAME, ast_name, "Multi-declaration is not allowed for types.");
		return;
	}
	...
	// 2025-12-17: Note regarding struct type naming: only structs used directly as entity declaration value are
	//             named. Otherwise the name is generated. Due to possible recursive struct use, we create structs
	//             as forward declared prototypes which are completed later. The name is provided during incomplete
	//             struct creation (here) and kept when struct is later completed during analyze pass.
	...
	struct id       *user_id       = &ast_name->data.ident.id;
	struct mir_type *fwd_decl_type = create_type_struct_incomplete(ctx, user_id, has_base);

	struct mir_instr *fwd_decl = create_instr_const_type(ctx, ast_value, fwd_decl_type);
	analyze_instr_rq(ctx, fwd_decl);

	// Set current fwd decl
	ctx->codegen->current_fwd_struct_decl = fwd_decl;

	struct mir_instr_block *init_block;
	struct mir_instr_block *prev_block = ast_initializer_block_begin(ctx, &init_block);

	// Generate initialization value in scope of init block.
	struct mir_instr *init = ast(ctx, ast_value);
	set_current_block(ctx, prev_block);
	...
	var->entry          = register_symbol(ctx, ast_name, id, scope, is_compiler);

	// Append initializer into the init block.
	set_current_block(ctx, init_block);
	append_instr_set_initializer(ctx, ast_name, decl, init);
	ast_initializer_block_end(ctx, prev_block);

	// Struct decl cleanup.
	ctx->codegen->current_fwd_struct_decl = NULL;
}

static void ast_decl_var_global(struct context *ctx, struct ast *ast_global) {
	struct ast *ast_names = ast_global->data.decl.name;
	struct ast *ast_type  = ast_global->data.decl.type;
	struct ast *ast_value = ast_global->data.decl_entity.value;
	...
	// Generate is scope of initializer block.
	struct mir_instr *value = ast(ctx, ast_value);
	...
	for (usize i = 0; i < sarrlenu(names); ++i) {
		...
		if (use_unroll) {
			init = append_instr_unroll(ctx, ast_name, init, (u32)i, false);
		}
...
struct mir_instr *ast_decl_entity(struct context *ctx, struct ast *entity) {
	...
	if (is_fn_decl) {
		...
		if (is_struct_decl) {
			ast_decl_var_struct(ctx, entity);
		} else if (is_global) {
			ast_decl_var_global(ctx, entity);
		} else {
...
struct mir_instr *ast_decl_arg(struct context *ctx, struct ast *arg) {
	...
	if (ast_value) {
		...
		if (ast_value->kind == AST_CALL_LOC) {
			...
			struct mir_instr_block *prev_block = ast_initializer_block_begin(ctx, NULL);
			{
				struct mir_instr *src = ast(ctx, ast_value);
				append_instr_set_initializer(ctx, ast_value, value, src);
			}
			ast_initializer_block_end(ctx, prev_block);
		}
	} else {
	...
	return append_instr_decl_arg(ctx,
	                             &(append_instr_decl_arg_args_t){
	                                 ...
	                                 .entry                 = entry,
	                                 .generation_call       = ctx->instantiate.call,
	                                 .is_inside_declaration = ctx->codegen->is_inside_fn_declaration,
	                                 .is_inside_recipe      = ctx->codegen->is_inside_recipe,
	                             });
}
...
struct mir_instr *ast_ref(struct context *ctx, struct ast *ref) {
	...
	if (next) {
		...
		return append_instr_member_ptr(ctx, .node = ref, .target_ptr = target, .ident = ident);
	}
	...
	return append_instr_decl_ref(ctx, .node = ref, .parent_unit = unit, .rid = &ident->data.ident.id, .scope = scope, .scope_layer = scope_layer);
}
...
struct mir_instr *ast_type_fn(struct context *ctx, struct ast *type_fn) {
	...
	const bool   is_polymorph  = isflag(type_fn->data.type_fn.flavor, AST_TYPE_FN_FLAVOR_POLYMORPH) && !ctx->instantiate.is_active;
	...
	return append_instr_type_fn(ctx, type_fn, ret_type, args, is_polymorph, ctx->codegen->is_inside_fn_declaration);
}
...
struct mir_instr *ast_type_fn_group(struct context *ctx, struct ast *group) {
	...
	struct id *id = group->data.type_fn_group.user_id;
...
struct mir_instr *ast_type_arr(struct context *ctx, struct ast *type_arr, s64 override_len) {
	...
	if (override_len == -1) {
		...
		len = append_instr_const_int(ctx, ast_len, ctx->builtin_types->t_s64, (u64)override_len, false);
	}

	struct mir_instr *elem_type = ast(ctx, ast_elem_type);
	struct id        *id        = type_arr->data.type_arr.user_id;
...
struct mir_instr *ast_type_struct(struct context *ctx, struct ast *type_struct) {
	...
	struct mir_instr *fwd_decl            = ctx->codegen->current_fwd_struct_decl;
	ctx->codegen->current_fwd_struct_decl = NULL;

	ast_nodes_t *ast_members    = type_struct->data.type_strct.members;
	const bool   is_union       = type_struct->data.type_strct.is_union;
	const bool   is_multi_value = type_struct->data.type_strct.is_multi_value;
	...
	if (ast_base_type) {
		...
		// @Note 2026-01-22: Base member is commited later when struct type is analyzed.
		provide_builtin_member(ctx, scope, base_member, false);
...
struct mir_instr *ast_type_poly(struct context *ctx, struct ast *poly) {
	...
	mir_types_t *queue       = ctx->instantiate.queue;
	s32         *queue_index = &ctx->instantiate.index;
	...
	if (ctx->instantiate.is_active) {
		if (sarrlen(queue) == (*queue_index)) {
			...
			scope_entry->as.type = replacement_type;
	...
	// anywhere inside function argument list declaration; even before ?T appears in
	...
	scope_entry->kind            = SCOPE_ENTRY_TYPE;
	scope_entry->as.type         = slave_type;
...
USE_DUMMY:
	clear_poly_replacement(ctx);
	return append_instr_const_type(ctx, poly, ctx->builtin_types->t_s32);
}

// Returns previous active block.
struct mir_instr_block *ast_initializer_block_begin(struct context *ctx, struct mir_instr_block **out_init_block) {
	struct mir_instr_block *prev_block = ast_current_block(ctx);
	struct mir_instr_block *block      = append_global_block(ctx, INIT_VALUE_FN_NAME);
	set_current_block(ctx, block);

	if (out_init_block) *out_init_block = block;
	return prev_block;
}

void ast_initializer_block_end(struct context *ctx, struct mir_instr_block *prev_block) {
	struct mir_instr_block *block = ast_current_block(ctx);
	if (!is_block_terminated(block)) terminate_block(block, block->last_instr);
	set_current_block(ctx, prev_block);
}
...
struct mir_instr *ast_create_expr_resolver_call(struct context *ctx, str_t fn_name, struct mir_type *fn_type, struct ast *ast_expr) {
	...
	fn->type                      = fn_type;
	struct mir_instr_block *entry = append_block(ctx, NULL, fn, cstr("entry"), false);
	...
	set_current_block(ctx, prev_block);
	struct mir_instr *call = create_instr_call(ctx, .node = ast_expr, .callee = fn_proto, .is_comptime = true);
...
struct mir_instr *ast(struct context *ctx, struct ast *node) {
	...
	case AST_EXPR_LIT_FN:
		return ast_expr_lit_fn(ctx, .lit_fn = node);
...
static void provide_builtin_arch(struct context *ctx) {
	...
	for (usize i = 0; i < static_arrlenu(arch_names); ++i) {
		...
		name       = scdup2(ctx->string_cache, name);
...
void mir_arenas_init(struct mir_arenas *arenas, u32 owner_thread_index) {
...
#if BL_DEBUG_ENABLE
	static bool sizes_reported = false;
	if (!sizes_reported) {
		sizes_reported = true;
		blog("Size reports");
		blog("\tMIR instruction: %lluB", instr_size);
		blog("\tMIR function:    %lluB", sizeof(struct mir_fn));
#if BL_PLATFORM_LINUX && BL_COMPILER_GNUC
		static_assert(SIZEOF_MIR_INSTR <= 232, "MIR instruction size grown.");
		static_assert(sizeof(struct mir_fn) <= 280, "MIR function size grown.");
#endif
	}
#endif
	...
	arena_init(&arenas->fn_recipe, sizeof(struct mir_fn_recipe), alignment_of(struct mir_fn_recipe), PREALLOC_BASE, owner_thread_index, (arena_elem_dtor_t)&fn_poly_dtor);
	...
	arena_init(&arenas->codegen, sizeof(struct mir_codegen), alignment_of(struct mir_codegen), PREALLOC_BASE, owner_thread_index, NULL);
	arena_init(&arenas->fn_dyncall, sizeof(struct mir_fn_dyncall), alignment_of(struct mir_fn_dyncall), PREALLOC_BASE, owner_thread_index, NULL);
	arena_init(&arenas->fn_instance, sizeof(struct mir_fn_instance), alignment_of(struct mir_fn_instance), PREALLOC_BASE, owner_thread_index, NULL);
}
...
void mir_arenas_terminate(struct mir_arenas *arenas) {
	...
	arena_terminate(&arenas->fn_recipe);
	arena_terminate(&arenas->fn_dyncall);
	arena_terminate(&arenas->fn_instance);
	arena_terminate(&arenas->codegen);
}
...
static void init_context(struct context *ctx, struct assembly *assembly) {
	...
	ctx->codegen                      = duplicate_codegen(ctx, NULL);
	ctx->codegen->current_scope_layer = SCOPE_DEFAULT_LAYER;
}

static void terminate_context(struct context *UNUSED(ctx)) {
}
...
static void initialize_builtins(struct assembly *assembly) {
	...
	// evaluation in comptime functions.
	assembly->is_comptime_run = add_builtin_global_bool(&ctx, &builtin_ids[BUILTIN_ID_IS_COMPTIME_RUN], true, false);

	// Add IS_COMPTIME immutable into the global scope this variable is overriden to true in scope
	// of comptime functions.
	assembly->is_comptime = add_builtin_global_bool(&ctx, &builtin_ids[BUILTIN_ID_IS_COMPTIME], true, false);
...
void mir_init(struct assembly *assembly) {
	...
	const u32 thread_index     = get_worker_index();
	mir->analyze.unnamed_entry = scope_create_entry(&assembly->thread_local_contexts[thread_index].scope_thread_local, .kind = SCOPE_ENTRY_UNNAMED, .is_builtin = true);
...
void mir_terminate(struct assembly *assembly) {
	...
	mtx_destroy(&mir->type_cache_lock);
...
#if BL_DEBUG_ENABLE
	// Emit mir in debug mode even in case compilation failed.
	if (assembly->target->emit_mir && builder.errorc) {
...
BL_EXPORT struct mir_type *__create_enum_type(char *names_ptr[], s64 names_len) {
	if (names_len < 1) babort("Invalid number of enum variants (%lld) provided to '__create_enum_type'.", names_len);

	bassert(current_context);
	struct context       *ctx = current_context;
	struct builtin_types *bt  = ctx->builtin_types;

	struct id *ids = bmalloc(sizeof(*ids) * names_len); // @Leaks

	struct scope   *scope    = scope_create(ctx->scope_thread_local, SCOPE_TYPE_ENUM, ctx->assembly->gscope, NULL);
	mir_variants_t *variants = arena_alloc(ctx->small_array_arena);
	for (s64 i = 0; i < names_len; ++i) {
		str_t               name    = scdup2(ctx->string_cache, make_str_from_c(names_ptr[i]));
		struct mir_variant *variant = create_variant(ctx, id_init(&ids[i], name), bt->t_s32, i);
		sarrput(variants, variant);
		provide_builtin_variant(ctx, scope, variant);
	}

	return create_type_enum(ctx, .scope = scope, .base_type = bt->t_s32, .variants = variants);
}

BL_EXPORT struct mir_type *__get_struct_member_type(struct mir_type *type, s64 index) {
	bmagic_assert(type);
	return mir_get_struct_elem_type(type, (usize)index);
}
